...
class FizzGame extends StatefulWidget {
  const FizzGame({Key? key}) : super(key: key);

  @override
  State<FizzGame> createState() => _FizzGameState();
}

class _FizzGameState extends State<FizzGame> {
          ...
          child: Column(
            children: [
              ...
              SizedBox(
                height: 350,
                child: FutureBuilder(
                    future: FirebaseService.fizzMatchHistory(),
                    builder: (context, snapshot) {
                      if (snapshot.hasData) {
                        var data = snapshot.data;
                        List matchList = [];
